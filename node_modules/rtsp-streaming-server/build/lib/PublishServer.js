...
class PublishServer {
  ...
  async start() {
    return new Promise((resolve, reject) => {
      this.server.listen(this.rtspPort, () => {
        console.log('Now listening on %s', this.rtspPort);
        return resolve();
      });
    });
  }
  ...
  optionsRequest(req, res) {
    console.log('Options request from %s with headers %o', req.socket.remoteAddress, req.headers);
  ...
  async announceRequest(req, res) {
    console.log('%s:%s - Announce request with headers %o', req.socket.remoteAddress, req.socket.remotePort, req.headers); // Ask for authentication

    if (this.hooks.authentication) {
      if (!req.headers.authorization) {
        console.log('%s:%s - No authentication information (required), sending 401', req.socket.remoteAddress, req.socket.remotePort);
        ...
        if (!result) {
          console.log('%s:%s - Invalid authentication information (required), sending 401', req.socket.remoteAddress, req.socket.remotePort);
          res.setHeader('WWW-Authenticate', 'Basic realm="rtsp"');
          res.statusCode = 401;
          return res.end();
        }
        ...
        if (!allowed) {
          console.log('%s:%s - Invalid authentication information (Hook returned false), sending 401', req.socket.remoteAddress, req.socket.remotePort);
          res.setHeader('WWW-Authenticate', 'Basic realm="rtsp"');
          res.statusCode = 401;
          return res.end();
        }
    ...
    req.on('end', async () => {
      ...
      if (this.hooks.checkMount) {
        ...
        if (!allowed) {
          console.log('%s:%s path not allowed by hook', req.socket.remoteAddress, req.socket.remotePort, req.uri);
          res.statusCode = 403;
          return res.end();
        }
      }
      ...
      if (mount) {
        console.log('%s:%s - Mount already existed, sending 503: %o', req.socket.remoteAddress, req.socket.remotePort, req.uri);
        // res.statusCode = 503;
        // return res.end();
      ...
      console.log('%s:%s - Set session to %s', req.socket.remoteAddress, req.socket.remotePort, mount.id);

      if(process.env.LIVE555_INTEGRATION!=null && process.env.LIVE555_INTEGRATION.toString()=='1')
      try {
        let URL = req.uri;
        let URLsplit = URL.split('/');
        URLsplit[2] = '172.17.0.1:9001'
        let newURL = URLsplit[0] + "//" + URLsplit[2] + "/" + URLsplit[3];
      ...
      else
      {
        let URL = req.uri;
        let URLsplit = URL.split('/');
        global.currentListOfStreams.push(
          {
            streamID: URLsplit[3],
            outputURL:req.uri.replace('9000','9001')
          }
        )
        console.log(global.currentListOfStreams)
      }
  ...
  setupRequest(req, res) {
    ...
    if (!mount) {
      console.log('%s:%s - No mount with path %s exists', req.socket.remoteAddress, req.socket.remotePort, req.uri);
    ...
    if (req.headers.transport && req.headers.transport.toLowerCase().indexOf('tcp') > -1) {
      console.log('%s:%s - TCP not yet supported - sending 501', req.socket.remoteAddress, req.socket.remotePort, req.uri);
      res.statusCode = 501; // Not Implemented
  ...
  async recordRequest(req, res) {
    ...
    if (!mount || mount.id !== req.headers.session) {
      console.log('%s:%s - No mount with path %s exists, or the session was invalid', req.socket.remoteAddress, req.socket.remotePort, req.uri);
  ...
  teardownRequest(req, res) {
    ...
    console.log('%s:%s - teardown %s', req.socket.remoteAddress, req.socket.remotePort, req.uri);
    this.mounts.deleteMount(req.uri);

    // FFmpeg teardown, clear input from live555
    if(process.env.LIVE555_INTEGRATION!=null && process.env.LIVE555_INTEGRATION.toString()=='1')
    this.onTeardownClearLive555(req.uri);
    else
    {
      let URL = req.uri;
      let URLsplit = URL.split('/');
      global.currentListOfStreams = global.currentListOfStreams.filter(stream => stream.streamID != URLsplit[3]);
      console.log(global.currentListOfStreams)

      //Ban data ve cho webhook de bao client.
      httpServer.http_server_event.emit('stream_removed', {
        streamID:  URLsplit[3]});
    }
  ...
  async onTeardownClearLive555(url) {
    try {
      let URLsplit = url.split('/');
      URLsplit[2] = '172.17.0.1:9001'
      let newURL = URLsplit[0] + "//" + URLsplit[2] + "/" + URLsplit[3]; 
  ...
  checkAuthenticated(req, res) {
    if (this.hooks.authentication && this.authenticatedHeader) {
      if (req.headers.authorization !== this.authenticatedHeader) {
        console.log('%s:%s - auth header mismatch (401) %O', req.socket.remoteAddress, req.socket.remotePort, req.headers);
        res.statusCode = 401;
        res.end();
        return false;
      }
