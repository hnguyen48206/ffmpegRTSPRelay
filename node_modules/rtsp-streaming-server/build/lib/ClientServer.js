...
class ClientServer {
  ...
  constructor(rtspPort, mounts, hooks) {
    ...
    this.server = (0, _rtspServer.createServer)((req, res) => {
      ...
      console.log('Client Request For RTSP')
      console.log(req.uri, req.method)
      //From this uri, we can do some sort of access control
  ...
  async playRequest(req, res) {
    ...
    //When Client is allowed to PLAY then this would be counted as a connection.
    res.end();
  }
  /**
   *
  ...
  async teardownRequest(req, res) {
    ...
    const client = this.clients[req.headers.session];
    client.close();
    //When Client is CLOSE then this would be counted as a connection removal.
    res.end();
  }
  /**
   *
